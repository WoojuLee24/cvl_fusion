...
if __name__ == '__main__':
    ...
    train_loader = dataset.get_data_loader('train', shuffle=True)  # or 'train' ‘val’
    ...
    for idx, data in enumerate(tqdm(train_loader)):
        ...
        # mask = data['query']['points3D'][:, :, 2] < 240.0
        # data['query']['points3D'] = data['query']['points3D'][mask]
        points3D_list.append(data['query']['points3D'])
    points3D_all = torch.cat(points3D_list, dim=0)
    mean = [points3D_all[:, :, 0].mean(), points3D_all[:, :, 1].mean(), points3D_all[:, :, 2].mean()]
    std = [points3D_all[:, :, 0].std(), points3D_all[:, :, 1].std(), points3D_all[:, :, 2].std()]

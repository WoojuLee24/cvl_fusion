...
class TwoViewRefiner3D(BaseModel):
    default_conf = {
        ...
        'optimizer': {
            'name': 'nn_optimizer3dv2_2tp', # 'learned_optimizer', #'basic_optimizer',
            'input': 'res',
            'pose_loss': 'triplet', # 'rr', 'none'
            'pose_lambda': 1,
            'delta_loss': 'none',   # 'delta_lm'
            'delta_lambda': 1,
            'main_loss': 'reproj',
            'coe_lat': 1.,
            'coe_lon': 1.,
    ...
    def _forward(self, data):
        ...
        for i in reversed(range(len(self.extractor.scales))):
            ...
            if self.conf.optimizer.multi_pose > 1:
                ...
                T_opt, failed, delta_list = opt(dict(
                    p3D=p3D_query, F_ref=F_ref, F_q=F_q, T_init=T_init, camera=cam_ref,
            ...
            if self.conf.optimizer.pose_loss == 'triplet': #pose_loss:
                ...
                pred['pose_loss'].append(diff_loss)
            if 'delta_lm' in self.conf.optimizer.delta_loss:
                delta_loss = 0
                for deltas in delta_list:
                    res = deltas[0] - deltas[1]
                    err = (res ** 2)
                    err = scaled_barron(1., 2.)(err)[0] / 4
                    err = torch.max(torch.zeros_like(err), err)
                    delta_loss += err.sum(dim=-1)
                pred['delta_loss'].append(delta_loss)
        return pred
    ...
    def reproj_loss(self, pred, data):
        ...
        if self.conf.optimizer.pose_loss != 'none':
            losses['pose_loss'] = 0
        if self.conf.optimizer.delta_loss != 'none':
            losses['delta_loss'] = 0
        ...
        for i, T_opt in enumerate(pred['T_q2r_opt']):
            ...
            if self.conf.optimizer.pose_loss != 'none':
                ...
                losses['total'] += (poss_loss_weight * pred['pose_loss'][i] / num_scales).clamp(
                    max=self.conf.clamp_error / num_scales)
            if self.conf.optimizer.delta_loss != 'none':
                losses['delta_loss'] += pred['delta_loss'][i]
                losses['total'] += self.conf.optimizer.delta_lambda * pred['delta_loss'][i]

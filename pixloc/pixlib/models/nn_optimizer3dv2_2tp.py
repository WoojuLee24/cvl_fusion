...
class NNOptimizer3D(BaseOptimizer):
    ...
    def _run(self, p3D: Tensor, F_ref: Tensor, F_query: Tensor,
        ...
        for i in range(self.conf.num_iters):
            ...
            delta_LM = optimizer_step(g, H, lambda_, mask=~failed)
            ...
            if self.conf.delta_loss == 'delta_lm':
                T_delta_list.append([delta, delta_LM])
            elif self.conf.delta_loss == 'delta_lm_detach':
                T_delta_list.append([delta, delta_LM.detach()])
            else:
                T_delta_list = None

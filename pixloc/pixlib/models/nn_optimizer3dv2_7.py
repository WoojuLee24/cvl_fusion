...
class NNOptimizer3D(BaseOptimizer):
    ...
    def _run(self, p3D: Tensor, F_ref: Tensor, F_query: Tensor,
        ...
        for i in range(self.conf.num_iters):
            ...
            if self.conf.integral:
                if self.conf.spatial == 4:
                    res_sum = []
                    trans_range = shift_range[0, 0]
                    shifts = [[-0.5, 0], [0.5, 0], [0, -0.5], [0, 0.5]]
                    for shift in shifts:
                        dx, dy = shift
                        translation_offset = torch.tensor([[1., 0, 0, dx * trans_range], [0, 1, 0, -dy * trans_range], [0, 0, 1, 0], [0, 0, 0, 1]], dtype=torch.float32).to(res.device)
                        translation_offset = Pose.from_4x4mat(translation_offset)
                        T_ = translation_offset @ T

                        res_, valid_, w_unc_, _, _, _ = self.cost_fn.residual_jacobian3(T_, *args)
                        res_sum.append(res_)
                    res_sum = torch.cat(res_sum, dim=-1)
                    self.nnrefine.r_sum = res_sum
...
class NNrefinev1_0(nn.Module):
    def __init__(self, args):
        ...
        if self.args.integral:
            ...
            if self.args.spatial == 4:
                self.cin = [c+I_size[i]*4 for i, c in enumerate(self.cin)]
            else:
    ...
    def forward(self, res, query_feat, ref_feat, p3D_query, p3D_ref, J, w_unc, valid, scale,
        ...
        if self.args.integral:
            ...
            r = torch.cat([r, self.args.ki * self.r_sum], dim=-1)

...
class NNrefinev1_0(nn.Module):
    def __init__(self, args):
        ...
        if self.args.geo_version in [1.0]:
            self.gin = [c * 3 for c in self.cin]

            self.geo_linear0 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[0], self.cout))
            self.geo_linear1 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[1], self.cout))
            self.geo_linear2 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[2], self.cout))
        elif self.args.geo_version in [1.01]:
            self.gin = [c * 3 for c in self.cin]

            self.geo_linear0 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[0], self.gin[0]),
                                             nn.ReLU(inplace=False),
                                             nn.Linear(self.gin[0], self.cout))
            self.geo_linear1 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[1], self.gin[1]),
                                             nn.ReLU(inplace=False),
                                             nn.Linear(self.gin[1], self.cout))
            self.geo_linear2 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[2], self.gin[2]),
                                             nn.ReLU(inplace=False),
                                             nn.Linear(self.gin[2], self.cout))
        elif self.args.geo_version in [1.1, 1.2]:    # spconv
            if self.args.geo_version == 1.1:
                mode = 'subm0.1'
            elif self.args.geo_version == 1.2:
                mode = 'subm0.2'
            self.gin = [c * 3 for c in self.cin]
            ...
            self.geo_linear2 = nn.Sequential(nn.ReLU(inplace=True),
                                             nn.Linear(self.gin[2], self.cout))
            max_num_features = self.args.max_num_points3D + self.args.max_num_out_points3D
    ...
    def forward(self, res, query_feat, ref_feat, p3D_query, p3D_ref, J, w_unc, valid, scale,
        ...
        geo = torch.cat([query_feat, ref_feat, p3D_ref_feat], dim=-1)
        if 2 - scale == 0:
            geo = self.geo_linear0(geo)
        elif 2 - scale == 1:
            geo = self.geo_linear1(geo)
        elif 2 - scale == 2:
            geo = self.geo_linear2(geo)

        if self.args.geo_version in [1.1, 1.2]:
            p3D_ref_ = self.grid_processor.process(p3D_ref)
            geo = self.spconv(geo, p3D_ref_, spatial_shape=self.args.voxel_shape, batch_size=B)

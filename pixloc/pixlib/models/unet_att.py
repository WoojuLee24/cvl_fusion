...
class UNet(BaseModel):
    ...
    def _forward(self, data):
        ...
        # for attention
        if self.conf.compute_attention and self.conf.compute_uncertainty:
            attended = []
            for idx, i in enumerate(self.conf.output_scales):
                att = out_features[idx] * confidences[idx]
                attended.append(att)
            pred['attended'] = attended
